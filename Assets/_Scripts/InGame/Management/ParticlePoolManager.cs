...
public class ParticlePoolManager : MonoBehaviour
{
    ...
    // 키에 따라 매핑될 파티클 프리팹 정보를 저장할 수 있는 클래스
    [System.Serializable]
    public class ParticleEntry
    {
        public string key;              // 파티클 식별 키
        public ParticleSystem prefab;  // 해당 키에 연결된 파티클 프리팹
    }

    [Header("등록된 파티클 프리팹 목록")]
    public ParticleEntry[] particlePrefabs; // 인스펙터에서 설정 가능한 파티클 배열

    // 키별 오브젝트 풀을 저장하는 딕셔너리
    ...
    private void Awake()
    {
        ...
        foreach (var entry in particlePrefabs)
        {
            ...
            particlePools[key] = new ObjectPool<ParticleSystem>(
                ...
                actionOnGet: (ps) => ps.gameObject.SetActive(true),    // 풀에서 가져올 때 활성화
                actionOnRelease: (ps) => ps.gameObject.SetActive(false), // 반환 시 비활성화
                actionOnDestroy: (ps) => Destroy(ps.gameObject),        // 풀 객체 파괴 시 게임 오브젝트 파괴
