...
public class SpawnerSelector : MonoBehaviour
{
    ...
    private Vector3 CalculateTargetPosition(NoteInfo currentNote)
    {
        ...
        // 인접 노트가 아닐 경우 또는 인접 노트이지만 이전 노트와 충분히 떨어진 위치를 가져야 할 경우의 기본 위치
        Vector3 defaultRandomTargetPos = new Vector3(
        ...
        if (_lastSpawnedNoteTime != -1.0f && (currentNote.time - _lastSpawnedNoteTime <= copyNoteDataSecOffset))
        {
            ...
            Vector3 finalCalculatedPos = basePos; // 기준 위치로 초기화

            switch (currentNote.requiredDirection)
            {
                case NoteDirection.Up:
                    // 위로 이동 시, PLAYABLE_Y_MAX를 넘지 않도록 클램핑
                    finalCalculatedPos.y = Mathf.Min(basePos.y + moveAmount, PLAYABLE_Y_MAX);
                    break;
                case NoteDirection.Down:
                    // 아래로 이동 시, PLAYABLE_Y_MIN을 넘지 않도록 클램핑
                    finalCalculatedPos.y = Mathf.Max(basePos.y - moveAmount, PLAYABLE_Y_MIN);
                    break;
                case NoteDirection.Left:
                    // 왼쪽으로 이동 시, currentPlayableXMin을 넘지 않도록 클램핑
                    finalCalculatedPos.x = Mathf.Max(basePos.x - moveAmount, currentPlayableXMin);
                    break;
                case NoteDirection.Right:
                    // 오른쪽으로 이동 시, currentPlayableXMax를 넘지 않도록 클램핑
                    finalCalculatedPos.x = Mathf.Min(basePos.x + moveAmount, currentPlayableXMax);
                    break;
                    // NoteDirection.Any는 AssignDirectionsToNotes에서 제거되었으므로 여기서는 처리되지 않습니다.
            }

            // 최종적으로 계산된 위치를 다시 한번 플레이 가능 영역 내로 클램핑하여 안전성 확보
            finalCalculatedPos.x = Mathf.Clamp(finalCalculatedPos.x, currentPlayableXMin, currentPlayableXMax);
            finalCalculatedPos.y = Mathf.Clamp(finalCalculatedPos.y, PLAYABLE_Y_MIN, PLAYABLE_Y_MAX);
            finalCalculatedPos.z = PLAYABLE_Z; // Z축은 고정

            return finalCalculatedPos;
        }
        else // 인접 노트가 아닐 경우 (시간 간격이 충분히 멀 때)
        {
            // 이전 노트 위치(_lastCalculatedTargetPos)를 기준으로 충분히 멀리 떨어진 랜덤 위치를 생성합니다.
            // 이렇게 하면 새로운 흐름이 시작될 때 이전 노트와 겹치지 않도록 할 수 있습니다.
            Vector3 targetPos = defaultRandomTargetPos;

            // 만약 _lastCalculatedTargetPos가 유효한 값이라면 (첫 노트가 아닐 경우)
            // 이전 위치를 기준으로 랜덤 오프셋을 더하는 방식도 고려해 볼 수 있습니다.
            // 현재는 defaultRandomTargetPos를 반환하므로, 이는 완전히 새로운 랜덤 시작점입니다.
            // 필요에 따라 이 로직을 조절하여 더 부드러운 전환을 만들 수 있습니다.
